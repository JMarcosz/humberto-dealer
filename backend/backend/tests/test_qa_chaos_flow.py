...
def test_flujo_completo_0_a_100(client):
    ...
    # 2. Búsqueda de disponibilidad para retiro en ventana operativa (2 horas) por 4 días
    f_ini = (datetime.utcnow() + timedelta(hours=2)).strftime("%Y-%m-%dT%H:%M")
    f_fin = (datetime.utcnow() + timedelta(hours=98)).strftime("%Y-%m-%dT%H:%M")
...
def test_chaos_devolucion_combustible_faltante(client):
    ...
    """
    f_ini = (datetime.utcnow() + timedelta(hours=2)).strftime("%Y-%m-%dT%H:%M")
    f_fin = (datetime.utcnow() + timedelta(hours=74)).strftime("%Y-%m-%dT%H:%M")
