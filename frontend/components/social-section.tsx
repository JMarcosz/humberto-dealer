...
const SOCIALS = [
  {
    label: 'Instagram',
    handle: '@humbertoautoimportsrl',
    url: INSTAGRAM_URL,
    icon: InstagramIcon,
    gradient: 'from-purple-600 via-pink-500 to-orange-400',
    desc: 'Síguenos para ver nuestro inventario actualizado, tips de compra y las mejores ofertas.',
  },
  {
    label: 'TikTok',
    handle: '@humbertoautoimport',
    url: TIKTOK_URL,
    icon: TikTokIcon,
    gradient: 'from-black via-gray-900 to-gray-700',
    desc: 'Mira nuestros videos de vehículos, recorridos y contenido exclusivo de la concesionaria.',
  },
  {
    label: 'WhatsApp',
    ...
    <section className="py-16 bg-background">
      <div className="container mx-auto px-4 sm:px-6 md:px-16">
        <div className="mb-10 text-center">
          ...
          <div className="mx-auto mt-3 h-px w-14 bg-orange-500" />
        </div>

        <div className="grid gap-5 sm:grid-cols-3">
          {SOCIALS.map(({ label, handle, url, icon: Icon, gradient, desc }) => (
            <a
              key={label}
              href={url}
              target="_blank"
              rel="noopener noreferrer"
              className="group relative overflow-hidden rounded-2xl border border-border/40 p-6 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl"
              style={{
            ...
            >
              ...
              <div
                className={`absolute inset-0 bg-gradient-to-br ${gradient} opacity-0 transition-opacity duration-300 group-hover:opacity-10 rounded-2xl`}
              />

              {/* Ícono con gradiente */}
              <div className={`mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br ${gradient} text-white shadow-lg`}>
                <Icon />
              </div>

              <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground">{label}</p>
              <p className="mt-0.5 text-base font-bold text-foreground">{handle}</p>
